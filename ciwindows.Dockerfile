...
go build -pgo=cpu.pprof -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${REVISION} -X main.date=${BUILDTIME}" -o /out/bin/upgraderr.exe
...
COPY --from=app-builder /out/bin/upgraderr* /

ENTRYPOINT ["C:\\upgraderr.exe"]
